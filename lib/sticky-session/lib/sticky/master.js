...
function Master(workerCount, options, masterPort) {
  ...
  this.once('listening', function() {
    ...
    for (var i = 0; i < workerCount; i++)
      this.spawnWorker(masterPort + i + 1);
  });
}
...
Master.prototype.spawnWorker = function spawnWorker(workerPort) {

  var worker = cluster.fork({...this.env, port: workerPort});
